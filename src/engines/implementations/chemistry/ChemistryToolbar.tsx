...
import type { ChemistryTopic } from './chemistryTypes';
import { Atom, Table, Sparkles, Scale, FlaskConical, RotateCcw } from 'lucide-react';

export const ChemistryToolbar: React.FC = () => {
  const { activeTopic, setTopic, resetAll } = useChemistryStore();

  const topicOptions: { id: ChemistryTopic; label: string; icon: any }[] = [
    { id: 'atom_structure', label: 'Bohr Atom Model', icon: Atom },
    { id: 'periodic_table', label: 'Periodic Table', icon: Table },
    { id: 'reactions', label: 'Reaction Simulator', icon: Sparkles },
    { id: 'balancing', label: 'Equation Balancer', icon: Scale },
    { id: 'acids_bases', label: 'Acids & Bases pH', icon: FlaskConical }
  ];
