...
interface BiologyFormulaPanelProps {
  presentationMode?: boolean;
}

export const BiologyFormulaPanel: React.FC<BiologyFormulaPanelProps> = ({ presentationMode = false }) => {
  const { activeTopic, params } = useBiologyStore();
  const { steps } = calculateBiology(activeTopic, params);

  if (steps.length === 0) return null;

  return (
    <div className={`w-full max-w-md bg-slate-900/90 backdrop-blur rounded-2xl border border-slate-800 p-4 shadow-2xl ${presentationMode ? 'scale-105 border-emerald-500/40' : ''}`}>
      <h3 className="text-xs font-bold text-slate-300 uppercase tracking-wider border-b border-slate-800 pb-2 mb-3 flex items-center justify-between">
        <span>Biology KaTeX Step Solver</span>
        <span className="text-[10px] text-emerald-400 font-mono">Formula → Substitution → Answer</span>
      </h3>
